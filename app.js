...
// ============================================================
const Auth = {
  // Call GAS backend to verify the Google ID token and get user info
  async signInWithGoogle() {
    // This triggers the Google One Tap / OAuth flow via GAS
    // GAS handles the OAuth redirect; it returns a session token we store locally
    const result = await API.post('auth_google', {});
    if (result.success) {
